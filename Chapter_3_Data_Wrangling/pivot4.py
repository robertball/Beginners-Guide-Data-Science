...
# example 1:
p = df.pivot_table(index='title', columns='sex', values='birth_year', aggfunc=[np.mean, 'count'], fill_value=0)
print(p)

# example 2:
p = df.pivot_table(index=['title', 'first_name'], columns=['sex', 'last_name'], values='birth_year', aggfunc=[np.mean, 'count'], fill_value=0)
print(p)
